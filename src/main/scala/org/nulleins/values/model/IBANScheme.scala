...
object IBANScheme extends AccountNumber {
  ...
  def validateChecksum(iban: String) = if ( checksumValid(iban)) Success(iban) else Failure("Invalid checksum")
  def generateChecksum(cc: ISO3166, bban: String) = 98 - mod97of(s"${cc}00$bban")
  def checksumValid(iban: String) = mod97of(iban) == BigInt(1)
  private def mod97of(iban: String) = BigInt(((iban drop 4) + (iban take 4)).map { cc =>
